...
library(ggsci) # Color palettes for scientific publications
...
**Additional Intermediate Data**
   ...
   1. [Genomic relationship matrices](http://nce.ads.uga.edu/wiki/lib/exe/fetch.php?media=uga_3_relationship.pdf) (**GRMs**) for each subgroup post-quality control using **G**enome-wide **C**omplex **T**rait **A**nalysis (**GCTA**)
...
tibble(Subgroup = c(1:12),
       ...
       `Code and File Name Abbreviation` = c('c', 'd', 'l', 'y', 'cd', 'cl', 'cy', 'dl', 'dy', 'ly', 'dly', 'cdly')) %>%
   kable(.,
         row.names = FALSE,
         align = 'c',
...
```{r, echo=FALSE, fig.align='center', fig.cap="Example of a PLINK PED file.", out.width="50%"}
include_graphics("images/figure_3_1.PNG")
```
...
```{r, echo=FALSE, fig.align='center', fig.cap="Example of a PLINK MAP file.", out.width="50%"}
include_graphics("images/figure_3_2.PNG")
```
...
The following script (\@ref(processplinkf90)) will fix these errors, along with row-binding each batch file into a single PED file for each subgroup:
...
The last modification of the genotype files will be filtering for genotype quality control. The following filters will be applied using PLINK:
  ...
  1. Remove SNPs with a [genotype call rate](https://onlinelibrary.wiley.com/doi/10.1002/sim.6605#:~:text=The%20call%20rate%20for%20a,less%20than%205%25%20missing%20data.) less than 90%
  2. Remove SNPs with a [minor allele frequency (**MAF**)](https://en.wikipedia.org/wiki/Minor_allele_frequency#:~:text=Minor%20allele%20frequency%20(MAF)%20is,occurs%20in%20a%20given%20population.&text=MAF%20is%20widely%20used%20in,rare%20variants%20in%20the%20population.) less than 0.01
  3. Remove individual pigs with genotype call rates less than 90%
  
The bash script below (\@ref(quality-control-2)) performs genotype filtering for quality control in each subgroup. The BED, BIM, and FAM files that are output from this script will be intermediate files used in all subsequent analyses.

```{bash quality-control-ex, eval=FALSE}
...
A genomic relationship matrix (GRM) is a matrix that estimates the genomic similarity between each animal in a genotype file based on their SNP haplotypes. [GCTA](https://cnsgenomics.com/software/gcta/#Overview) has a [function](https://cnsgenomics.com/software/gcta/#MakingaGRM) to estimate this matrix based off of the method described by [Yang et al., 2010](https://www.nature.com/articles/ng.608). Below is the command that was invoked to produce a GRM for subgroup 1 (entire bash script is in \@ref(makegrms)):

```{bash makegrms-ex, eval=FALSE}
# /data/cjgwx7/tml_gpsm/scripts/bash/
...
Below is an example of this script (ran in R; \@ref(makepheno)) for subgroup 5 (Crossbred and Duroc populations), which will show how each phenotype filetype is made:

```{r make-pheno-ex, eval=FALSE}
library(data.table)
library(dplyr)
